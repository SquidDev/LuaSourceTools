--- Type definitions used to check LuaInspect itself.
-- @module luainspect.typecheck
...
return function(context)
	...
	local ast = T.table {
		tag = T.string,
		lineinfo=T.table{
			first=T.table{comments=T.table{T.table{T.string,T.number,T.number}},T.number,T.number,T.number,T.string},
	...
	ast.localdefinition=ast
	ast.localmasking = ast
	ast.previous = ast
	ast.parent = ast
	ast.seevalue = ast
	ast.seenote=ast
