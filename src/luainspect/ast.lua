--[[-- Lua Abstract Syntax Tree (AST) and token list operations.
...
function M.get_keywords(ast, src)
	...
	while i <= #ast do
		...
		if i == 0 then	-- before first child
			...
			fpos = (c and c.lineinfo and c.lineinfo.last or last).offset + 1
		end
		local lpos
		if j == #ast+1 then	-- after last child
			...
			lpos = (c and c.lineinfo and c.lineinfo.first or first).offset - 1
		end
