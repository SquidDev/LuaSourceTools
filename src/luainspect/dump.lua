--- Custom dumper for objects
-- @module luainspect.dump

local M = { }
local insert = table.insert

M.DEFAULT_CFG = {
	-- Print the non-array part of tables?
	hide_hash   = false;
	-- Use Metalua's backtick syntax sugar?
	metalua_tag = true;
	-- Set of keywords which must not use Lua's field shortcuts {["foo"]=...} -> {foo=...}
	keywords    = { };
	-- Set of fields to not display
	blacklist = { };
	-- Max depth to traverse
	max_depth = -1;
	-- If the field 'name' exists, print that instead.
	with_name = false;
}
...
-- @tparam table cfg Convig to pass
-- @treturn string Formatted object
...
--- Format a string
-- @tparam string fmt Format to use
...
-- @tparam string fmt Format to print with
