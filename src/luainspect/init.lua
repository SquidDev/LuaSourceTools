--- Core LuaInspect source analysis.
--
...
--
-- @copyright 2010 David Manura, MIT License.
-- @module luainspect.init
...
function M.eval_comments(ast, tokenlist, report)
	...
	for idx=1,#tokenlist do
		...
		if token.tag == 'Comment' then
			local command = token[1]:match'^!(.*)' or token[1]:match'\n!(.*)'
			if command then
		...
		if info.retvals then
			...
			if vals.n == 0 then
				...
				for i=1,vals.n do local val = vals[i]
					ts[#ts+1] = T.istype[val] and tostring(val) or LD.tostring(val) --Q:dumpstring too verbose?
				end
...
--- Get the severity of a token
-- @treturn string? severity of warning (nil, "warn" or "error")
-- @treturn table? List of warnings
function M.get_messages(token, tokenlist, src)
