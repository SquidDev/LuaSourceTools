--- Convert AST to HTML using LuaInspect info embedded.
-- @module luainspect.html
	...
	if token then
		...
		if token.tag == 'Id' or ast.isfield then
			...
			if ast.lineinfo then
				local linenum = ast.lineinfo.first.line
				desc_html = desc_html .. '\nused-line:' .. linenum
			end

			local severity, warnings = LI.get_messages(token, tokenlist, src)
			if severity then
				class = class .. " " .. severity
				desc_html = desc_html .. '\n<p class="warnings">' .. escape_html(table.concat(warnings,"\n")) .. '</p>'
			end
