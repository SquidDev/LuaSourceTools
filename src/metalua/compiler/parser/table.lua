--- Parser for modules
-- @module metalua.compiler.parser.table
...
return function(M)
	...
	table.bracket_pair = gg.sequence{ "[", _expr, "]", "=", _expr, builder = "Pair" }

	--- table element parser: list value, `id = value` pair or `[value] = value` pair.
	function table.element(lx)
		if lx :is_keyword (lx:peek(), "[") then return table.bracket_pair(lx) end
		local e = M.expr(lx)
	...
	table.table = gg.sequence{ "{", _table.content, "}", builder = unpack }
